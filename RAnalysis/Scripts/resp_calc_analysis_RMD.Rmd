...
```
# Plot the rates by the size metrics here

```{r standardized rates by normalization metric , echo=TRUE}
library('ggpubr') # use ggarrange to export pdf fles

# by WHOLE animal dry weight  :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

P_WholeDW_resp_all  <- Resp.Master_OM[!is.na(Resp.Master_OM$resp_umol_L_hr),] %>% 
                          ggplot(aes(x = whole_Dry_weight, y = resp_umol_L_hr, color = as.factor(pH))) +
                            geom_smooth(method = "lm", se=FALSE, color="black", formula = resp_umol_L_hr ~ whole_Dry_weight) +
                            theme_classic() +
                            labs(title = "Whole Dry Weight (mg) v. Respiration (μmol L O2 hr)", y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%hr^{-1}~")")) +
                            xlab("Whole animal dry weight (mg)") +
                            geom_point()
...
P_WholeDW_resp_facet <- Resp.Master_OM[!is.na(Resp.Master_OM$resp_umol_L_hr),] %>% 
                          ggplot(aes(x = whole_Dry_weight, y = resp_umol_L_hr, color = as.factor(pH))) +
                            geom_smooth(method = "lm", se=FALSE, color="black", formula = resp_umol_L_hr ~ whole_Dry_weight) +
                            theme_classic() +
                            labs(y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%hr^{-1}~")")) +
                            xlab("Whole animal dry weight (mg)") +
                            geom_point() + 
...
ggarrange(P_WholeDW_resp_all, P_WholeDW_resp_facet,nrow = 2)
# Export to pdf
ggarrange(P_WholeDW_resp_all, P_WholeDW_resp_facet,nrow = 2) %>%
  ggexport(filename = "C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Respiration/Juvenile_respiration_FoodxOA_WholeDW_Standardized.pdf")

# by Length :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

P_Length_resp_all <- Resp.Master_OM[!is.na(Resp.Master_OM$resp_umol_L_hr),] %>% 
                        ggplot(aes(x = Length_um, y = resp_umol_L_hr, color = as.factor(pH))) +
                          geom_smooth(method = "lm", se=FALSE, color="black", formula = resp_umol_L_hr ~ Length_um) +
                          theme_classic() +
                          labs(title = "Shell length (μm) v. Respiration (μmol L O2 hr)", y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%hr^{-1}~")")) +
...
# Export to pdf
ggarrange(P_Length_resp_all, P_Length_resp_facet,nrow = 2) %>%
  ggexport(filename = "C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Respiration/Juvenile_respiration_FoodxOA_ShellLength_Standardized.pdf")
...
pdf(paste0("C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Respiration/All_respiration_FoodxOA_LengthStandardized.pdf"))
Resp_LengthStand
dev.off()
...
Resp_DryWgtStand   <- ggplot(data=RespSumTab_DW, aes(x=Age, y=resp_µmol_L_g_hr, color=pCO2)) +
          ...
          labs(title = "F1 Scallops: Food x OA Experiment", 
               y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%g^{-1}~whole~DW%.% hr^{-1}~")")) +
          scale_linetype_manual(values = c("High food" = "solid", "Low food" = "dashed"))  +
          theme(legend.position="none") +
          # facet_wrap(~factor(Fed_Unfed, level = c('Low food', 'High food'))) 
          facet_wrap(~pCO2)
...
kable(pander(summary(LMmod_0914_SL)), style='rmarkdown') 
...
kable(pander(anova(LMEmod_0930_DW), style='rmarkdown'))
# |     &nbsp;      | numDF | denDF | F-value |  p-value  |
# |:---------------:|:-----:|:-----:|:-------:|:---------:|
...
Res1026_Length_reps <- ggboxplot(Resp_01026, x = "pHxFood", y = "resp_µmol_L_mm_Length_hr", color = "Replicate",
                          add = c("mean_se", "dotplot"),
                          palette = c("#00AFBB", "#E7B800", "brown", "grey"))

require(nlme)
LMEmod_01026_SL<-lme(resp_µmol_L_mm_Length_hr ~ pCO2 * Food, random=~1|Replicate, data=Resp_01026)
...
LMmod_01026_SL <- aov(lm(resp_µmol_L_mm_Length_hr~pCO2*Food,data=Resp_01026))
...
shapiro.test(residuals(LMmod_01026_SL)) #  normal - 0.3875 (0.01626)
leveneTest(LMmod_01026_SL) # good - 0.6186 (0.01844 *)
...
Resp1026_Length <- ggplot(Resp_01026, aes(x = factor(pCO2, level = c('500 μatm', '800 μatm')), resp_µmol_L_mm_Length_hr , fill = pCO2)) +
              theme(panel.grid=element_blank()) +
              geom_boxplot(size=0.2, width=0.5, alpha=0.1, aes(fill=pCO2)) +
              scale_fill_manual(values=c("white","grey50")) +
              geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
...
# Export to pdf
ggarrange(Resp1026_Length, Res1026_Length_reps,nrow = 2) %>% 
  ggexport(filename = "C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Respiration/20211026_figs_tables/202101026_respiration_length.standardized.pdf")
...
library("ggpubr")
Resp_01026$pHxFood <- paste(Resp_01026$pH, Resp_01026$Food, sep = "_")
Res1026_WholeDW_reps <- ggboxplot(Resp_01026, x = "pHxFood", y = "resp_µmol_L_g_hr", color = "Replicate",
                          add = c("mean_se", "dotplot"),
                          palette = c("#00AFBB", "#E7B800", "brown", "grey"))
...
shapiro.test(residuals(LMmod_01026_DW)) #  normal - 0.01843
leveneTest(LMmod_01026_DW) # 0.01179 *
TukeyHSD(LMmod_01026_DW)
...
Res1026_WholeDW <- ggplot(Resp_01026, aes(x = factor(pCO2, level = c('500 μatm', '800 μatm')), resp_µmol_L_g_hr , fill = pCO2)) +
                            theme(panel.grid=element_blank()) +
                            geom_boxplot(size=0.2, width=0.5, alpha=0.1, aes(fill=pCO2)) +
                            scale_fill_manual(values=c("white","grey50")) +
                            geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
...
# Export to pdf
ggarrange(Res1026_WholeDW, Res1026_WholeDW_reps,nrow = 2) %>% 
  ggexport(filename = "C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Respiration/20211026_figs_tables/202101026_respiration_DW.standardized.pdf")
