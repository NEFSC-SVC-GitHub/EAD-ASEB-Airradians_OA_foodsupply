...
### Skip to here to load the resp master file (if already comopleted above chumks!!)
...
df_mean <- Resp_0930 %>% 
  ...
  dplyr::summarize(average = mean(resp_µmol_L_m_Length_hr)) %>%
  dplyr::ungroup()
p.box <- ggplot(Resp_0930, aes(x = pHxFood, y = resp_µmol_L_m_Length_hr)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxFood, y = average),
             color="red") 
