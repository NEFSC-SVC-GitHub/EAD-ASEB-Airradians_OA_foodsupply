...
``` {r loops for for statements - visualization of slope and calculation loops, echo = TRUE}
# Day 0 0914 
...
# Clearance rate anlaysis for 10/26 data
...
write.csv(meanBLANKS.Summary, "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/Blanks_Master.csv")
...
LMEmod_0914_FR<-lme(FeedingRate_L_hour_m ~ pH, random=~1|random_fact, data=df_total.914)
...
df_total.930 <- df_total.930  %>% 
  dplyr::mutate(pCO2 = case_when(pH == 8.0 ~ "500 μatm", pH == 7.5 ~ "800 μatm")) %>% 
  dplyr::mutate(random_fact = as.factor(paste(pH,Replicate,Fed_Unfed, sep = '_'))) %>% 
...
df_total.930 %>%  summarySE(measurevar="FeedingRate_L_hour_m", groupvars=("Fed_Unfed")) 
...
LMEmod_0930_FR<-lme(FeedingRate_L_hour_m ~ pH * Fed_Unfed, random=~1|random_fact, data=df_total.930)
# |      &nbsp;      | numDF | denDF | F-value |  p-value  |
# |:----------------:|:-----:|:-----:|:-------:|:---------:|
...
kable(as.data.frame(anova(LMEmod_0930_FR))) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20210930_figs_tables/20210930_FR_LME_Table.png", zoom = 1.5)
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20210930_figs_tables/20210930_FR_LME_Diagnostics.pdf"))
...
# density plot - Day 64 filtration rate 
```{r, Day 64 density length plot }
library(dplyr)
library(tidyr)

df_total.930$pHxfood <- as.factor(paste(df_total.930$pH, df_total.930$Fed_Unfed, sep = "_"))
df_mean <- df_total.930 %>% 
  dplyr::group_by(pHxfood) %>% 
  dplyr::summarize(average = mean(FeedingRate_L_hour_m)) %>%
  dplyr::ungroup()
p.box <- ggplot(df_total.930, aes(x = pHxfood, y = FeedingRate_L_hour_m)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxfood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  ...
  mutate(pHxfood = factor(x, labels = levels(df_total.930$pHxfood), ordered = TRUE)) %>%
  select(-x)

denisty_box_FR_64DPF <-ggplot(p.box.data) +
                            ...
                            # density plot
                            geom_density(data = df_total.930, aes(x = FeedingRate_L_hour_m, fill=pHxfood, ..scaled..), alpha=.4) +
                            #  theme
                            theme_classic() +
                            xlim(0, 30) +
                            facet_grid(pHxfood ~ .) +
                            theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none", plot.title = element_text(size=10))+
                            labs(x = "Filtration rate", y = "frequency")  +
                            scale_fill_manual(values=c("grey40", "grey40", "grey85", "grey85"))
denisty_box_FR_64DPF
# export the plot
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20210930_figs_tables/20210930_FR_density_plot.pdf"), width = 4, height = 8)
denisty_box_FR_64DPF # print the model diagnostics 
dev.off()
```
...
df_total.1026  %>% summarySE(measurevar="FeedingRate_L_hour_m", groupvars=("Fed_Unfed")) 
...
require(nlme)
# Linear mixed effects model with random effect of Replicate 
LMEmod_1026_FR<-lme(FeedingRate_L_hour_m ~ pH * Fed_Unfed, random=~1|random_fact, data=df_total.1026  )
# |      &nbsp;      | numDF | denDF | F-value |  p-value  |
# |:----------------:|:-----:|:-----:|:-------:|:---------:|
...
kable(as.data.frame(anova(LMEmod_1026_FR))) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20211026_figs_tables/20211026_FR_LME_Table.png", zoom = 1.5)
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20211026_figs_tables/20211026_FR_LME_Diagnostics.pdf"))
...
df_total.1026$pHxfood <- as.factor(paste(df_total.1026$pH, df_total.1026$Fed_Unfed, sep = "_"))
df_mean <- df_total.1026 %>% 
  dplyr::group_by(pHxfood) %>% 
  dplyr::summarize(average = mean(FeedingRate_L_hour_m)) %>%
  dplyr::ungroup()
p.box <- ggplot(df_total.1026, aes(x = pHxfood, y = FeedingRate_L_hour_m)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxfood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  select(x, ymin, lower, middle, upper, ymax, outliers, flipped_aes) %>%
  dplyr::rename(mean = flipped_aes) %>% 
  dplyr::mutate(mean  = (df_mean$average)) %>% 
  mutate(pHxfood = factor(x, labels = levels(df_total.1026$pHxfood), ordered = TRUE)) %>%
  select(-x)

ggplot(df_total.1026, aes(x=FeedingRate_L_hour_m, color=pHxfood)) +
  geom_density()


denisty_box_FR_92DPF <-ggplot(p.box.data) +
                            # manually plot flipped boxplot
                            geom_segment(aes(x = ymin, xend = ymax, y = -0.2, yend = -0.2)) +
                            geom_rect(aes(xmin = lower, xmax = upper, ymin = -0.25, ymax = -0.15,  fill = pHxfood),color = "black") +
                            geom_point(data = . %>% unnest(outliers), aes(x = outliers, y = -0.2), color = "grey60", size = 1) +
                            ...
                            geom_vline(data = . %>% select(pHxfood, lower, mean, upper) %>% gather(key, value, -pHxfood), aes(xintercept = value)) +
                            # density plot
                            geom_density(data = df_total.1026, aes(x = FeedingRate_L_hour_m, fill=pHxfood, ..scaled..), alpha=.4) +
                            #  theme
                            theme_classic() +
                            xlim(0, 30) +
                            facet_grid(pHxfood ~ .) +
                            ...
                            labs(x = "Filtration rate", y = "frequency")  +
                            scale_fill_manual(values=c("grey40", "grey40", "grey85", "grey85"))
denisty_box_FR_92DPF
# export the plot
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/FeedingRates/20211026_figs_tables/20211026_FR_density_plot.pdf"), width = 4, height = 8)
denisty_box_FR_92DPF # print the model diagnostics 
dev.off()
```
