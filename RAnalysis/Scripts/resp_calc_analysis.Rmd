...
###############################################################################################################################3
### Rerun the analysis WITHOUT this data point assuming this appears to be an outlier 
omitMAX <- max(Resp_1026$resp_µmol_L_mm_Length_hr)
Resp_1026_OM <- Resp_1026 %>% dplyr::filter(!resp_µmol_L_mm_Length_hr > (omitMAX-0.001)) # now 31 rows as opposed to 32 (ommited the outlier)

Resp1026_LengthOM <- ggplot(Resp_1026_OM, aes(x = factor(pCO2, level = c('500 μatm', '800 μatm')), resp_µmol_L_mm_Length_hr , fill = pCO2)) +
              theme(panel.grid=element_blank()) +
              geom_boxplot(size=0.2, width=0.5, alpha=0.1, aes(fill=pCO2)) +
              scale_fill_manual(values=c("white","grey50")) +
              geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
              ...
              labs(title = "F1 Scallops: respiration rates on 20211026 (shell length stand.)", 
                   ...
                   x = expression(italic(p)*CO[2]~Treatment~"("~μ*atm~")"))  + 
              facet_wrap(~factor(Fed_Unfed, level = c('Low food', 'High food'))) # +
Resp1026_LengthOM #ommited the outlier


# Linear mixed effects model with random effect of Replicate 
LMEmod_1026_SLOM<-lme(resp_µmol_L_mm_Length_hr ~ pCO2 * Food, random=~1|random_fact, data=Resp_1026_OM)
# |     &nbsp;      | numDF | denDF | F-value |  p-value  |
# |:---------------:|:-----:|:-----:|:-------:|:---------:|
...
pander(anova(LMEmod_1026_SLOM), style='rmarkdown')
shapiro.test(residuals(LMEmod_1026_SLOM)) #  normal - 0.9677 - omitting the outlier resolved normality!


kable(as.data.frame(anova(LMEmod_1026_SLOM))) %>% 
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/20211026_resp_LME_Table.png", zoom = 1.5)
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/20211026_resp_LME_Diagnostics.pdf"))
check_model(LMEmod_1026_SLOM) 
dev.off()

# Export to pdf
ggarrange(Resp1026_LengthOM, Res1026_Length_reps,nrow = 2) %>% 
...
Resp_1026_OM$resp_µmol_L_mm_Length_hr.NUM       <- as.numeric(sprintf("%s", Resp_1026_OM$resp_µmol_L_mm_Length_hr.noSCI))

df_mean <- Resp_1026_OM %>% 
  dplyr::group_by(pHxFood) %>% 
  dplyr::summarize(average = mean(resp_µmol_L_mm_Length_hr.NUM)) %>%
  dplyr::ungroup()
p.box <- ggplot(Resp_1026_OM, aes(x = pHxFood, y = resp_µmol_L_mm_Length_hr.NUM)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxFood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  select(x, ymin, lower, middle, upper, ymax, outliers, flipped_aes) %>%
  dplyr::rename(mean = flipped_aes) %>% 
  dplyr::mutate(mean  = df_mean$average) %>% 
  mutate(pHxFood = factor(x, labels = levels(Resp_1026_OM$pHxFood), ordered = TRUE)) %>%
  select(-x)
