...
###  density plot - Day 64; b factor FOOD 

```{r, Day 14 density plot - b factor FOOD}
library(dplyr)
library(tidyr)
Resp_0930$pHxFood <- as.factor(Resp_0930$pHxFood)
...
df_mean <- Resp_0930 %>% 
  dplyr::group_by(pHxFood) %>% 
  dplyr::summarize(average = mean(resp_umol_hr_bFactorNormLength.FOOD)) %>%
  dplyr::ungroup()
p.box <- ggplot(Resp_0930, aes(x = pHxFood, y = resp_umol_hr_bFactorNormLength.FOOD)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxFood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  select(x, ymin, lower, middle, upper, ymax, outliers, flipped_aes) %>%
  dplyr::rename(mean = flipped_aes) %>% 
...
denisty_box_resp_64DPF <-ggplot(p.box.data) +
                            ...
                            geom_vline(data = . %>% select(pHxFood, lower, mean, upper) %>% gather(key, value, -pHxFood), aes(xintercept = value)) +
                            # density plot
                            geom_density(data = Resp_0930, aes(x = resp_umol_hr_bFactorNormLength.FOOD, group=pHxFood, fill=pHxFood, ..scaled..), alpha=.4, adjust=1.5) +
                            #  theme
                            theme_classic() +
                            xlim(0, 0.22) +
                            facet_grid(pHxFood ~ .) +
                            theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none", plot.title = element_text(size=10))+
                            labs(title = "F1 Scallops: respiration rates 64 DPF - b factor FOOD", 
                                 x = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%mm^{-1}~shell~length%.% hr^{-1}~")"),
                                 y = "Scaled density") +
                            scale_fill_manual(values=c("grey40", "grey40", "grey85", "grey85"))
denisty_box_resp_64DPF
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20210930_figs_tables/20210930_RRbfactorFOOD_density_plot.pdf"), width = 5, height = 5)
denisty_box_resp_64DPF # print the model diagnostics 
dev.off()
...
Resp_1026 %>% summarySE(measurevar="resp_umol_hr_bFactorNormLength.ALL", groupvars=c("pCO2","Fed_Unfed"))

Resp_1026 %>% summarySE(measurevar="resp_umol_hr_bFactorNormLength.FOOD", groupvars=c("pCO2","Fed_Unfed"))
```
...
# |       &nbsp;       | Df |  Sum Sq   |  Mean Sq  | F value | Pr(>F) |
# |:------------------:|:--:|:---------:|:---------:|:-------:|:------:|
# |      **pCO2**      | 1  | 0.0002251 | 0.0002251 | 0.3335  | 0.5682 |
# |   **Fed_Unfed**    | 1  | 0.0005036 | 0.0005036 | 0.7461  | 0.3951 |
# | **pCO2:Fed_Unfed** | 1  | 0.0001806 | 0.0001806 | 0.2675  | 0.609  |
# |   **Residuals**    | 28 |  0.0189   | 0.0006751 |   NA    |   NA   |
# output results 
kable(data.frame(unclass(summary(AOV1026.bfactALL)), 
                   check.names = FALSE, 
                   stringsAsFactors = FALSE)  ) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/20211026_AOV_RR_bfactor_ALL.png", zoom = 1.5)

# Linear mixed effects model with random effect of Replicate 
LME1026.bfactALL<-lme(resp_umol_hr_bFactorNormLength.ALL ~ 
                     pCO2*Fed_Unfed, 
                     random=~1|Chamber_tank, # with the random effect as an LME model (Chamber_tank = replicate as 7.5_A, 7.5_B)
                     data=Resp_1026) 
# check model assumptions
shapiro.test(residuals(LME1026.bfactALL)) # normal - 0.3702
leveneTest(residuals(LME1026.bfactALL) ~ Resp_1026$pCO2) # 0.3248 
leveneTest(residuals(LME1026.bfactALL) ~ Resp_1026$Fed_Unfed) # 0.54 
# AIC model selection
pander(AIC(AOV1026.bfactALL, LME1026.bfactALL), style='rmarkdown') # anova table of lmer
# |        &nbsp;        | df |  AIC   |
# |:--------------------:|:--:|:------:|
# | **AOV1026.bfactALL** | 5  | -137.1 | # for negative AIC, the lower the AIC the best fit the model - the simple AOV is better 
# | **LME1026.bfactALL** | 6  | -104.7 |
...
Resp1026_Length <- ggplot(Resp_1026, 
                    ...
                    scale_x_discrete(labels= c('~500 μatm', '~800 μatm')) +
                   # scale_y_continuous(expand = c(0, 0), limits = c(0, 2)) +
                    labs(title = "F1 Scallops: respiration rates on 202101026 (b factor ALL)", 
                         y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%mm~shell~length^{-1}%.% hr^{-1}~")"),
                         x = expression(italic(p)*CO[2]~Treatment~"("~μ*atm~")")) + 
                    theme(axis.text=element_text(size=8),
                          axis.title=element_text(size=8,face="bold")) +
...
# Visualize the data with the random effect of tank replicate 
Resp1026_bLengthFOOD_reps <- ggboxplot(Resp_1026, 
                                 x = "pHxFood", 
                                 y = "resp_umol_hr_bFactorNormLength.FOOD", 
                                 color = "Replicate",
          add = c("mean_se", "dotplot"),
          palette = c("#00AFBB", "#E7B800", "brown", "grey"))
Resp1026_bLengthFOOD_reps

# One-way ANOVA (without the random effect) 
AOV1026.bfactFOOD <- aov(resp_umol_hr_bFactorNormLength.FOOD ~ 
                       pCO2 * Fed_Unfed, 
                     data=Resp_1026) # run mod
# check model assumptions
shapiro.test(residuals(AOV1026.bfactFOOD)) # normal - 0.9948
leveneTest(AOV1026.bfactFOOD) # heterogeneity of variance  -0.00197 **
pander(aov(AOV1026.bfactFOOD), style='rmarkdown') # table for anova test
# |       &nbsp;       | Df |  Sum Sq   |  Mean Sq  | F value |  Pr(>F)   |
# |:------------------:|:--:|:---------:|:---------:|:-------:|:---------:|
# |      **pCO2**      | 1  | 2.15e-05  | 2.15e-05  | 0.02844 |  0.8673   |
# |   **Fed_Unfed**    | 1  |  0.0107   |  0.0107   |  14.16  | 0.0007914 |
# | **pCO2:Fed_Unfed** | 1  | 9.506e-06 | 9.506e-06 | 0.01257 |  0.9115   |
# |   **Residuals**    | 28 |  0.02117  | 0.0007561 |   NA    |    NA     |
SRH1026.bfactFOOD <- scheirerRayHare(resp_umol_hr_bFactorNormLength.FOOD ~ 
                       pCO2 * Fed_Unfed, 
                     data=Resp_1026) # run mod
# all is good, lets see the results
pander(print(SRH1026.bfactFOOD), style='rmarkdown') # table for SRH test
# |       &nbsp;       | Df | Sum Sq |   H    | p.value  |
# |:------------------:|:--:|:------:|:------:|:--------:|
# |      **pCO2**      | 1  |  28.9  | 0.3285 |  0.5666  |
# |   **Fed_Unfed**    | 1  | 852.9  | 9.692  | 0.001851 |
# | **pCO2:Fed_Unfed** | 1  | 25.48  | 0.2895 |  0.5905  |
# |   **Residuals**    | 28 |  1819  |   NA   |    NA    |
# output results 
kable(data.frame(unclass(print(SRH1026.bfactFOOD)), 
                   check.names = FALSE, 
                   stringsAsFactors = FALSE)  ) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/202101026_SRH_RR_bfactor_FOOD.png", zoom = 1.5)

# Linear mixed effects model with random effect of Replicate 
LME1026.bfactFOOD<-lme(resp_umol_hr_bFactorNormLength.FOOD ~ 
                     pCO2*Fed_Unfed, 
                     random=~1|Chamber_tank, # with the random effect as an LME model (Chamber_tank = replicate as 7.5_A, 7.5_B)
                     data=Resp_1026) 
# check model assumptions
shapiro.test(residuals(LME1026.bfactFOOD)) # normal - 0.9948
leveneTest(residuals(LME1026.bfactFOOD) ~ Resp_1026$pCO2) # 0.1469  
leveneTest(residuals(LME1026.bfactFOOD) ~ Resp_1026$Fed_Unfed) # 0.004791 **
# AIC model selection
pander(AIC(AOV1026.bfactFOOD, LME1026.bfactFOOD), style='rmarkdown') # anova table of lmer
# |        &nbsp;         | df |  AIC   |
# |:---------------------:|:--:|:------:|
# | **AOV1026.bfactFOOD** | 5  | -133.5 |# for negative AIC, the lower the AIC the best fit the model - the simple AOV is better 
# | **LME1026.bfactFOOD** | 6  | -101.5 |
...
# plot
Resp1026_Length_bfactFOOD <- ggplot(Resp_1026, 
                         aes(x = factor(pCO2, level = c('500 μatm', '800 μatm')), 
                             resp_umol_hr_bFactorNormLength.FOOD , 
                             fill = pCO2)) +
                    theme(panel.grid=element_blank()) +
                    ...
                    scale_x_discrete(labels= c('~500 μatm', '~800 μatm')) +
                   # scale_y_continuous(expand = c(0, 0), limits = c(0, 2)) +
                    labs(title = "F1 Scallops: respiration rates on 202101026 (b factor FOOD)", 
                         y = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%mm~shell~length^{-1}%.% hr^{-1}~")"),
                         x = expression(italic(p)*CO[2]~Treatment~"("~μ*atm~")")) + 
                    theme(axis.text=element_text(size=8),
                          axis.title=element_text(size=8,face="bold")) +
                    stat_summary(fun.y=mean, geom="point", shape=18, size=4, color="black", fill="white") + 
                    facet_wrap(~Fed_Unfed)  
Resp1026_Length_bfactFOOD # print this figure...

# Export the figures to pdf
ggarrange(Resp1026_Length_bfactFOOD,Resp1026_bLengthFOOD_reps,nrow = 2) %>%
  ggexport(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/20211026_RR_bfactor_FOOD.pdf")
...
df_mean <- Resp_1026 %>% 
  ...
  dplyr::summarize(average = mean(resp_umol_hr_bFactorNormLength.ALL)) %>%
  dplyr::ungroup()
p.box <- ggplot(Resp_1026, aes(x = pHxFood, y = resp_umol_hr_bFactorNormLength.ALL)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxFood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  select(x, ymin, lower, middle, upper, ymax, outliers, flipped_aes) %>%
  dplyr::rename(mean = flipped_aes) %>% 
...
denisty_box_resp_92DPF <-ggplot(p.box.data) +
                            ...
                            geom_vline(data = . %>% select(pHxFood, lower, mean, upper) %>% gather(key, value, -pHxFood), aes(xintercept = value)) +
                            # density plot
                            geom_density(data = Resp_1026, aes(x = resp_umol_hr_bFactorNormLength.ALL, group=pHxFood, fill=pHxFood, ..scaled..), alpha=.4, adjust=1.5) +
                            #  theme
                            theme_classic() +
                            xlim(0, 0.22) +
                            facet_grid(pHxFood ~ .) +
...
df_mean <- Resp_1026 %>% 
  ...
  dplyr::summarize(average = mean(resp_umol_hr_bFactorNormLength.FOOD)) %>%
  dplyr::ungroup()
p.box <- ggplot(Resp_1026, aes(x = pHxFood, y = resp_umol_hr_bFactorNormLength.FOOD)) + geom_boxplot() + geom_point(data= df_mean, mapping = aes(x = pHxFood, y = average),
             color="red") 
p.box.data <- layer_data(p.box) %>%
  select(x, ymin, lower, middle, upper, ymax, outliers, flipped_aes) %>%
  dplyr::rename(mean = flipped_aes) %>% 
...
denisty_box_resp_92DPF <-ggplot(p.box.data) +
                            ...
                            geom_vline(data = . %>% select(pHxFood, lower, mean, upper) %>% gather(key, value, -pHxFood), aes(xintercept = value)) +
                            # density plot
                            geom_density(data = Resp_1026, aes(x = resp_umol_hr_bFactorNormLength.FOOD, group=pHxFood, fill=pHxFood, ..scaled..), alpha=.4, adjust=1.5) +
                            #  theme
                            theme_classic() +
                            xlim(0, 0.22) +
                            facet_grid(pHxFood ~ .) +
                            theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none", plot.title = element_text(size=10))+
                            labs(title = "F1 Scallops: respiration rates 92 DPF - b factor FOOD", 
                                 x = expression(Respiration~rate~"("~μmol~L^{-1}~O[2]%.%mm^{-1}~shell~length%.% hr^{-1}~")"),
                                 y = "Scaled density") +
                            scale_fill_manual(values=c("grey40", "grey40", "grey85", "grey85"))
denisty_box_resp_92DPF
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/20211026_figs_tables/20211026_RRbfactorFOOD_density_plot.pdf"), width = 5, height = 5)
denisty_box_resp_92DPF # print the model diagnostics 
dev.off()
