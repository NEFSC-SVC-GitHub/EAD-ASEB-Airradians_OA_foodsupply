...
dups_to_add <- blanks_means[c(6,11),] %>% mutate(pH = 7.5) # only pH 8 contains a positive blank on 9/14 Run 1 and 10/26 Run 2- duplicate and save as pH 7.5 

blanks_means <- rbind(blanks_means, dups_to_add)
```
...
Resp.Master_OM <- Resp.Master[!is.na(Resp.Master$Length_um),] %>% dplyr::filter(!resp_blankStand < 0) %>% # ommit respiration values that are positive  
  ...
  dplyr::mutate(resp_mg_L_hr =(resp_blankStand *  # currently as just mg O2 per minute
...
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/Shell_length_standard_value.pdf"))
...
require(SciViews)
# look at the b value for each time point and fed or unfed treatment 
r_914 <- Resp.Master_OM %>%  dplyr::filter(Date %in% '9/14/2021')
ln_VO2.914 <- as.numeric(ln(r_914$resp_umol_L_hr)) # assign resp value
ln_SL.914  <- as.numeric(ln( (r_914$Length_um/1000) )) # assign length value 
model.914  <- lm(ln_VO2.914~ln_SL.914) # run model for r quared value
summary(model.914)
plot(ln_VO2.914~ln_SL.914) 


r_930 <- Resp.Master_OM %>%  dplyr::filter(Date %in% '9/30/2021') %>% dplyr::filter(Food %in% 'unfed')
ln_VO2.930 <- as.numeric(ln(r_930$resp_umol_L_hr)) # assign resp value
ln_SL.930  <- as.numeric(ln( (r_930$Length_um/1000) )) # assign length value 
model.930  <- lm(ln_VO2.930~ln_SL.930) # run model for r quared value
summary(model.930) # 1.07 for unfed; 1.53 for fed
plot(ln_VO2.930~ln_SL.930) 

r_1026 <- Resp.Master_OM %>%  dplyr::filter(Date %in% '10/26/2021') %>% dplyr::filter(Food %in% 'unfed')
ln_VO2.1026 <- as.numeric(ln(r_1026$resp_umol_L_hr)) # assign resp value
ln_SL.1026  <- as.numeric(ln( (r_1026$Length_um/1000) )) # assign length value 
model.1026  <- lm(ln_VO2.1026~ln_SL.1026) # run model for r quared value
summary(model.1026) # 1.2193 for unfed; 1.53  0.5270for fed
plot(ln_VO2.1026~ln_SL.1026) 



pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Respiration/Shell_length_resp_allometric_coefficient.pdf"))
plot(ln_SL,ln_VO2) 
abline(model)
...
P_SLStzd_resp_all  <- Resp.Master_OM[!is.na(Resp.Master_OM$resp_µmol_L_SL.StzedRate),] %>% 
                          ggplot(aes(x = resp_µmol_L_SL.StzedRate, y = resp_umol_L_hr, color = as.factor(pCO2))) +
                            scale_color_manual(values=c("forestgreen","darkorange2"))+
                            geom_smooth(method = "lm", se=FALSE, color="black", formula = resp_umol_L_hr ~ resp_µmol_L_SL.StzedRate) +
                            theme_classic() +
                            theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), plot.title = element_text(size=10))+ 
                            labs(title = "Raw Resp v. Standardized rate (shell l)", y = expression(μmol~L^{-1}~O[2]%.%hr^{-1})) +
                            xlab("Standardized rate") +
                            theme(legend.position="none") +
                            geom_point()

P_SLStzd_resp_facet <- Resp.Master_OM[!is.na(Resp.Master_OM$resp_µmol_L_SL.StzedRate),] %>% 
                          ggplot(aes(x = resp_µmol_L_SL.StzedRate, y = resp_umol_L_hr, color = as.factor(pCO2))) +
                            scale_color_manual(values=c("forestgreen","darkorange2"))+
                            geom_smooth(method = "lm", se=FALSE, color="black", formula = resp_umol_L_hr ~ resp_µmol_L_SL.StzedRate) +
                            theme_classic() +
