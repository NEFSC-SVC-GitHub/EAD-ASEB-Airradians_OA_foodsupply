...
for (i in 1:nrow(loop_914)) {
  ...
  if (nrow(dat2) > 0) {
    ClearRate.table           <- data.frame(matrix(nrow = nrow(dat2), ncol = 9)) # create dataframe to save cumunalitively during for loop
    colnames(ClearRate.table) <- c('Date', 'ID', 'pH', 'Replicate', 'Num','Run','Time_period', 'AlgaeLossRatio', 'ClearanceRate') # names for comuns in the for loop
    ...
    ClearRate.table$ClearanceRate <- dat2$ClearanceRate
...
ClearRates_914_Means <- df_total.914 %>% 
  dplyr::filter(!ClearanceRate %in% '-Inf') %>% 
  dplyr::group_by(pH, substr(Time_period,4,4)) %>% 
  dplyr::summarise(
    meanCR = mean(abs(ClearanceRate)),
    sdCR   = sd(abs(ClearanceRate)), 
    seCR   = sd(abs(ClearanceRate)) / sqrt(length(abs(ClearanceRate))), 
    n = n()) %>% 
  na.omit()
colnames(ClearRates_914_Means)[2] <- 'Time_period'

summary(lmer(meanCR~pH+ (1|Time_period), data=ClearRates_914_Means))
summary(aov(lm(meanCR~pH, data=ClearRates_914_Means)))
...
for (i in 1:nrow(loop_930)) {
    ...
    colnames(ClearRate.table) <- c('Date', 'ID', 'pH', 'Fed_Unfed', 'Replicate', 'Num','Run','Time_period', 'AlgaeLossRatio', 'ClearanceRate') # names for comuns in the for loop
...
df_total.930 %>% 
  ...
  dplyr::filter(!ClearanceRate %in% '-Inf') %>% 
  #dplyr::filter(!Time_period %in% c('40-50', '50-60')) %>% 
  ggplot(aes(pH_feed , abs(ClearanceRate) , fill = pH_feed)) +
  theme(panel.grid=element_blank()) +
