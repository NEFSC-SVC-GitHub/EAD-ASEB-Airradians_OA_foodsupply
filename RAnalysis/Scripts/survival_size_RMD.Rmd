...
LMEmod_CI_1026      <- lme(CI ~ pH, 
                           random=~1|random_eff, 
                           data= LMEmod_CI_1026.DATA) # run the model

LMERmod_CI_1026      <- lmer(CI ~ pH + 
                           (1|random_eff), 
                           data= LMEmod_CI_1026.DATA) # run the model
...
leveneTest(residuals(LMEmod_CI_1026) ~ LMEmod_CI_1026.DATA$pH) # 0.1017
LMEmod_CI_1026_T    <- lme(log(CI) ~ pH, 
                           random=~1|random_eff, 
                           data= LMEmod_CI_1026.DATA) # run the model
shapiro.test(residuals(LMEmod_CI_1026_T)) # non-normal 8.769e-07
leveneTest(residuals(LMEmod_CI_1026) ~ LMEmod_CI_1026.DATA$pH) # 0.1017
...
# |     &nbsp;      | numDF | denDF | F-value | p-value  |
# |:---------------:|:-----:|:-----:|:-------:|:--------:|
# | **(Intercept)** |   1   |  83   |  104.4  | 2.22e-16 |
# |     **pH**      |   1   |   6   |  7.589  | 0.03308  |
hist(resid(mod_shellwt_length))
qqnorm((resid(mod_shellwt_length)))
shapiro.test(resid(mod_shellwt_length)) #  p-value = 0.1
...
#First make calculations for means and standard error
LarvaeSurvival_Summ <- summarySE(survival_larvae, 
                                 measurevar="Survival", 
...
LarvaeSurvival_Summ$pCO2 <- factor(LarvaeSurvival_Summ$pCO2, 
                                   c("500 μatm","800 μatm", "1200 μatm"))
...
LarvaeSurvival_Plot_low_mod <- LarvaeSurvival_Summ %>% 
...
pdf(paste0("C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/Larval_survival.pdf"),width=10, height=7)
LarvaeSurvival_Plot
dev.off()

pdf(paste0("C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/Larval_survival_LowMod.pdf"))
LarvaeSurvival_Plot_low_mod
dev.off()
```
...
shapiro.test(residuals(ShellLengthMod_D51)) # 2.316e-14 non normal
leveneTest(residuals(ShellLengthMod_D51) ~ data_51$pCO2) # 0.8915
ShellLengthMod_D51_T = lme(log(Length_um) ~ pCO2, random=~1|random_fact,data=data_51)
shapiro.test(residuals(ShellLengthMod_D51_T)) # 2.316e-14 non normal
leveneTest(residuals(ShellLengthMod_D51_T) ~ data_51$pCO2) # 0.8915
...
data_64_AVG.length <- data_64_length %>%  summarySE(measurevar="Length_um", groupvars= c("pCO2", "Fed_Unfed"))
# data_64_AVG.length
# 
# data_64_AVG.length <- data_64_length %>%  summarySE(measurevar="Length_um", groupvars= "Fed_Unfed")
# data_64_AVG.length
...
data_64_AVG.survival <- data_64_survival %>%  summarySE(measurevar="Survival", groupvars= c("pCO2", "Fed_Unfed"))
data_64_AVG.survival

# Linear mixed effects models :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
require(nlme)

# Length:::::::::::::::::::::::::::::::::::::::::::::::
ShellLengthMod_D64 = lme(Length_um ~ 
                           pCO2 * Fed_Unfed, 
                         random=~1|random_fact,
                         data=data_64_length)
leveneTest(residuals(ShellLengthMod_D64) ~ data_64_length$pCO2) # 0.5277
shapiro.test(resid(ShellLengthMod_D64)) # < 2.2e-16
pander(anova(ShellLengthMod_D64), style='rmarkdown') # anova table of lmer
# output the model results
# |       &nbsp;       | numDF | denDF | F-value |  p-value  |
# |:------------------:|:-----:|:-----:|:-------:|:---------:|
# |  **(Intercept)**   |   1   | 1088  |  2112   |     0     |
# |      **pCO2**      |   1   |  12   | 0.03278 |  0.8593   |
# |   **Fed_Unfed**    |   1   |  12   |   107   | 2.488e-07 |
# | **pCO2:Fed_Unfed** |   1   |  12   | 0.07402 |  0.7902   |
kable(as.data.frame(anova(ShellLengthMod_D64))) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20210930_figs_table/20210930_length_LME_Table.png", zoom = 1.5)
# mod assumptions
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20210930_figs_table/20210930_length_LME_Diagnostics.pdf"))
...
# |       &nbsp;       | numDF | denDF | F-value |  p-value  |
# |:------------------:|:-----:|:-----:|:-------:|:---------:|
# |  **(Intercept)**   |   1   |  12   |  1005   | 6.127e-13 |
# |      **pCO2**      |   1   |  12   |  2.791  |  0.1207   |
# |   **Fed_Unfed**    |   1   |  12   | 0.1292  |  0.7255   |
# | **pCO2:Fed_Unfed** |   1   |  12   | 0.1006  |  0.7565   |
...
kable(as.data.frame(anova(SurvivalhMod_D64))) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20210930_figs_table/20210930_survival_LME_Table.png", zoom = 1.5)
# mod assumptions
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20210930_figs_table/20210930_survival_LME_Diagnostics.pdf"))
check_model(SurvivalhMod_D64) # print the model diagnostics 
dev.off()
shapiro.test(residuals(SurvivalhMod_D64)) # non normal 0.00368
leveneTest(residuals(SurvivalhMod_D64) ~ data_64_survival$pCO2) # 0.04072 *
leveneTest(residuals(SurvivalhMod_D64) ~ data_64_survival$Food) # 0.2942
...
# density plot - Day 64 shell length data 
...
data_92_length <- length %>%
  filter(Age_DPF=="92") %>%
  dplyr::mutate(pCO2 = 
                  case_when(pH == 8 ~ "500 μatm",  pH == 7.5 ~ "800 μatm")) %>% 
  dplyr::mutate(Fed_Unfed = 
                  case_when(Fed_Unfed == "fed" ~ "High food",  
                            Fed_Unfed == "unfed" ~ "Low food"))%>% 
  dplyr::mutate(random_fact = as.factor(paste(pCO2,Replicate,Fed_Unfed, sep = "_")))
...
data_92_AVG.length <- data_92_length %>%  
  summarySE(measurevar="Length_um", 
            groupvars= c("pCO2", "Fed_Unfed"))
# data_92_AVG.length

data_92_AVG.length <- data_92_length %>%  
  summarySE(measurevar="Length_um", groupvars= "Fed_Unfed")
# data_92_AVG.length
...
data_92_AVG.survival<- data_92_survival %>%  summarySE(measurevar="Survival", groupvars= "Fed_Unfed")
# data_92_AVG.survival
...
ShellLengthMod_D92 = lme(Length_um ~ 
                           pCO2 * Fed_Unfed, 
                         random=~1|random_fact,
                         data=data_92_length)
ShellLengthMod_D92 = lmer(Length_um ~ 
                           pCO2 * Fed_Unfed + (1|random_fact),
                          family="binomial",
                         data=data_92_length)
print(ShellLengthMod_D92, corr=F)


shapiro.test(residuals(ShellLengthMod_D92)) # non normal 6.026e-15
leveneTest(residuals(ShellLengthMod_D92) ~ data_92_length$pCO2) # 0.1068
leveneTest(residuals(ShellLengthMod_D92) ~ data_92_length$Fed_Unfed) # < 2.2e-16 ***
# output the model results
# |       &nbsp;       | numDF | denDF | F-value |  p-value  |
# |:------------------:|:-----:|:-----:|:-------:|:---------:|
# |  **(Intercept)**   |   1   |  549  |  1598   |     0     |
# |      **pCO2**      |   1   |  12   | 0.06353 |  0.8053   |
# |   **Fed_Unfed**    |   1   |  12   |  329.6  | 4.303e-10 |
# | **pCO2:Fed_Unfed** |   1   |  12   | 0.1953  |  0.6664   |
...
kable(as.data.frame(anova(ShellLengthMod_D92))) %>%  # print a png of this table
  kable_styling() %>%
  save_kable(file = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20211026_figs_table/20211026_length_LME_Table.png", zoom = 1.5)
# mod assumptions
pdf(paste0(filename = "C:/Users/samjg/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Output/Survival_size/20211026_figs_table/20211026_length_LME_Diagnostics.pdf"))
...
SurvivalhMod_D92 = lme(Survival ~ pCO2 * Food, random=~1|random_fact, data=data_92_survival) # no replicates within the tanks a,b,c,d - linear model used here
# output the model results
...
# density plot - Day 92 shell length data 
```{r, Day 92 density length plot }
library(dplyr)
library(tidyr)
