...
survival                   <-read.csv(file="C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Data/Survival_Size/Survival_50DPF_92DPF.csv", header = TRUE)
survival_larvae            <-read.csv(file="C:/Users/samuel.gurr/Documents/Github_repositories/Airradians_OA-foodsupply/RAnalysis/Data/Survival_Size/Survival_2d_18d.csv", header = TRUE)
...
```{r Length plot Mean SE, include=FALSE}
...
len.dryweight.CI_1202$Rep_pH <- as.character(paste(len.dryweight.CI_1202$Tank_Replicate, len.dryweight.CI_1202$pH, sep = '_'))

lmer(Condition_index_drywtRatio_Walne1976 ~ pH, + (1|Rep_pH),
                       data=len.dryweight.CI_1202,
                       REML=TRUE)
...
Tissue <- dryweight_CI %>% 
  ...
  dplyr::rename(c(type = shell.or.tissue, tissue_weight = weight))
Shell <- dryweight_CI %>% 
  dplyr::select(c("shell.or.tissue", "weight")) %>%
  filter(shell.or.tissue %in% "shell") %>%
  dplyr::rename(c(type = shell.or.tissue, shell_weight = weight))
