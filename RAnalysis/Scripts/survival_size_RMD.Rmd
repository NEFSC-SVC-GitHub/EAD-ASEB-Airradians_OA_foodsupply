...
survival_juvenile <-read.csv(file="C:/Users/samjg/Documents/Github_repositories/Airradians_OA/RAnalysis/Data/Survival_Size/JuvenileSurvival_LengthMeans.csv", header = TRUE)
survival_larvae   <-read.csv(file="C:/Users/samjg/Documents/Github_repositories/Airradians_OA/RAnalysis/Data/Survival_Size/Survival_2d_18d.csv", header = TRUE)
...
# Shell Length
ShellLengthMod_D51 <- aov(lm(Length_um~pCO2, data = data_51) ) 
...
# Shell Length
ShellLengthMod_D92 <- aov(lm(Length_um~pCO2*Fed_Unfed, data = data_92) ) 
...
Length_92dpf <- ggplot(data_92, aes(x =pCO2, Length_um,  fill = pCO2)) +
  theme(panel.grid=element_blank()) +
  geom_boxplot(size=0.2, alpha=0.1, aes(fill=pCO2)) +
  scale_fill_manual(values=c("grey50","white")) +
  geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
