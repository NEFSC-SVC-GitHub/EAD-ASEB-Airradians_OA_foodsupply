...
df        <-read.csv(file="C:/Users/samjg/Documents/Github_repositories/Airradians_OA/RAnalysis/Data/Survival_Size/51_64_92_dpf_means.csv", header = TRUE)
df_larvae <-read.csv(file="C:/Users/samjg/Documents/Github_repositories/Airradians_OA/RAnalysis/Data/Survival_Size/Survival_2d_18d.csv", header = TRUE)
#to look at headers
head(df)
tail(df)
...
stL <- summarySE(df, measurevar="Survival", groupvars=c("Age_DPF", "pCO2", "Fed_Unfed"))
...
## Use geom_line and geom_point to plot over time
pPlot1<-ggplot(data=stL, aes(x=Age_DPF, y=Survival, color=pCO2)) +
  ...
  facet_wrap(~pCO2)
pPlot1
pdf(paste0("C:/Users/samjg/Documents/Github_repositories/Airradians_OA/RAnalysis/Output/Survival_size/Juvenile_survival_FoodxOA.pdf"))
pPlot1
dev.off()
...
```{r}
#First make calculations for means and standard error
stL <- summarySE(df, measurevar="Survival", groupvars=c("Age_DPF", "trt"))
stL
#na.omit(stL)
...
## Use geom_line and geom_point to plot over time
pPlot1<-ggplot(data=stL, aes(x=Age_DPF, y=Survival, color=trt)) +
  geom_line( stat = "identity", size=1.0)+
  geom_point()+ 
...
levels(data_92$pCO2) <- c("High", "Low")



# SizeMod_D92 <- aov(lm(Length~pCO2*Fed_Unfed, data = data_92) ) 
# pander(summary(SizeMod_D92), style='rmarkdown')
# check_model(SizeMod_D92) # observe the diagnostics of the model
# shapiro.test(residuals(SizeMod_D92)) # non normal
# leveneTest(SizeMod_D92) # good
# TukeyHSD(SizeMod_D92)
# 
...
# Length_92dpf <- ggplot(data_92, aes(x = factor(pCO2, level = c('Low', 'High')), Length , fill = pCO2)) +
#   theme(panel.grid=element_blank()) +
#   geom_boxplot(size=0.2, alpha=0.1, aes(fill=pCO2)) +
#   scale_fill_manual(values=c("grey50","white")) +
#   geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
...
Survival_92DPF <- ggplot(data_92, aes(x = factor(pCO2, level = c('Low', 'High')), (Survival*100) , fill = pCO2)) +
  theme(panel.grid=element_blank()) +
  geom_boxplot(size=0.2, alpha=0.1, aes(fill=pCO2)) +
  scale_fill_manual(values=c("grey50","white")) +
  geom_point(shape = 21, size = 2, position = position_jitterdodge(jitter.width = 0.1)) +
