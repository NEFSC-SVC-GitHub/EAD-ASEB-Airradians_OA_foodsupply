...
MeasuredValues_byDate <- as.data.frame(Measured_values %>% # means for all values (by CO2SYS calculation) and within day (mean of replicate A B C D for each pHxfood treatment)
    dplyr::filter(!pH %in% 7) %>% 
    group_by(Date, Fed_Unfed, pH) %>% # group by target variables
    select(-c('OA.pCO2.treat.', 'Calculation')) %>%  # omit unnecessary factors and Si and P are 0 across the board...
    na.omit() %>% # omit NA occurances in the values (occasionally no TA alue for that day...)
    summarise_each(funs(mean)))# get means as mean per day per treatment 
MeasuredValuesTable <- MeasuredValues_byDate %>% # shrink the previous dataset down to a single value - meaan for the whole experiment challenge (N = number of days)
    dplyr::select(-(Date)) %>% 
    group_by(Fed_Unfed, pH) %>% 
    summarise_each(funs(mean,sd,se=sd(.)/sqrt(n())))
MeasuredValuesTable_n <- as.data.frame(MeasuredValues_byDate %>%  # get n (# of days)
...
chemTable_byDate <- as.data.frame(CO2SYS_Master_CarbChem %>% # means for all values (by CO2SYS calculation) and within day (mean of replicate A B C D for each pHxfood treatment)
    dplyr::filter(!pH %in% 7) %>% 
...
chemTable <- chemTable_byDate %>% # shrink the previous dataset down to a single value - meaan for the whole experiment challenge (N = number of days)
    dplyr::select(-(Date)) %>% 
    group_by(Fed_Unfed, pH) %>%
    summarise_each(funs(mean,sd,se=sd(.)/sqrt(n())))
chemTable_n <- as.data.frame(chemTable_byDate %>%  # get n (# of days)
     dplyr::group_by(Fed_Unfed, pH) %>%
     dplyr::mutate(Fed_Unfed = case_when(Fed_Unfed == 'fed' ~ "High", Fed_Unfed == 'unfed' ~ "Low")) %>% 
     dplyr::summarise(n = n()))
...
MeasuredVals_master <- rbind(TA_MeanSE, TACO2_MeanSE, pChosenScale_MeanSE)

pd <- position_dodge(0.1) # adjust the jitter for the different treatments   

Fed_measuredvals <- ggplot(MeasuredVals_master, aes(x=Date, y=value, colour=as.factor(pH), group=as.factor(pH))) + 
